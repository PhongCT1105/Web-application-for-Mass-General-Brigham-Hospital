...
export const MapEditorRefactor: React.FC = () => {
  ...
  const addMarkers = useCallback((map: Map, nodesOnFloor: HospitalData[]) => {
  ...
  }, [addMarkers, hospitalData, memoizedFindLines]);
...
//     const { nodes, edges } = useGraphContext();
//     const [hospitalData, setHospitalData] = useState<HospitalData[]>([]);
//     const [currectHospitalData, setCurrentHospitalData] = useState<HospitalData[]>(hospitalData.filter(
//         (node) => node.floor == "1",
//     ));
//
//     useEffect(() => {
...
//
//
//     const memoizedFindLines = useCallback(findLines, [edges, hospitalData, findLines]);
//     useEffect(() => {
//         let map: Map | null = mapRef.current;
//         if (!map) {
//             map = L.map("map-container", {
...
//         clearLines();
//
//         const bounds: LatLngBoundsExpression = [ [0, 0], [3400, 5000] ];
//         L.imageOverlay(initialFloorImage, bounds).addTo(map);
//         map.setMaxBounds(bounds);
//
//         const newNodesOnCurrentFloor = hospitalData.filter((node) => node.floor === convertedFloorName);
//         addMarkers(map, newNodesOnCurrentFloor);
//
//         //
//         setCurrentHospitalData(newNodesOnCurrentFloor);
//         //
//
//         memoizedFindLines(newNodesOnCurrentFloor);
//     }
